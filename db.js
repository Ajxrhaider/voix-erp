...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY, password TEXT, fullname TEXT, role TEXT);
  CREATE TABLE IF NOT EXISTS customers (id TEXT PRIMARY KEY, name TEXT, ipAddress TEXT, location TEXT, serviceType TEXT, status TEXT, contact TEXT, billingStatus TEXT, joinedDate TEXT, interactionHistory TEXT);
  ...
  CREATE TABLE IF NOT EXISTS daily_reports (id TEXT PRIMARY KEY, submittedBy TEXT, dept TEXT, date TEXT, summary TEXT, highlights TEXT, status TEXT);
  CREATE TABLE IF NOT EXISTS employees (id TEXT PRIMARY KEY, name TEXT, dept TEXT, baseSalary REAL, currentScore INTEGER, salesComm REAL);
  CREATE TABLE IF NOT EXISTS appraisals (employeeId TEXT PRIMARY KEY, hrScore INTEGER, hodScore INTEGER, gmScore INTEGER);
  CREATE TABLE IF NOT EXISTS payroll_adjustments (employeeId TEXT PRIMARY KEY, penalties REAL, spoilage REAL, lateComing REAL, absenteeism REAL, damagedProperty REAL, salaryAdvance REAL, bonuses REAL, commissions REAL, overtime REAL, taxRate REAL, healthInsurance REAL, rentAdvance REAL);
  CREATE TABLE IF NOT EXISTS inventory (id TEXT PRIMARY KEY, item TEXT, qty REAL, unit TEXT, cost REAL);
  CREATE TABLE IF NOT EXISTS requisitions (id TEXT PRIMARY KEY, dept TEXT, item TEXT, qty INTEGER, estCost REAL, reason TEXT, status TEXT);
  
  -- UPDATED / NEW TABLES FOR CTO BRIDGE --
  CREATE TABLE IF NOT EXISTS work_orders (id TEXT PRIMARY KEY, teamId TEXT, ticketId TEXT, objective TEXT, materialsRequired TEXT, status TEXT);
...
// Seed Initial Data (Employees + Inventory)
const empCount = db.prepare("SELECT count(*) as count FROM employees").get();
if (empCount.count === 0) {
  const insertEmp = db.prepare("INSERT INTO employees (id, name, dept, baseSalary, currentScore, salesComm) VALUES (?, ?, ?, ?, ?, ?)");
  ...
  const insertInv = db.prepare("INSERT INTO inventory (id, item, qty, unit, cost) VALUES (?, ?, ?, ?, ?)");
